...
export default function TjansterPage() {
  return (
    <>
      <PageHeader
        eyebrow="Tjänster"
        title="Sex sätt vi kan hjälpa dig"
        ...
        <ul className="space-y-px bg-[var(--color-border)]">
          {services.map((s, i) => (
            <li
              key={s.slug}
              id={s.slug}
              className="scroll-mt-32 bg-[var(--color-paper)]"
            >
              <article className="grid gap-6 px-1 py-10 sm:px-0 lg:grid-cols-[auto_1fr_1fr] lg:gap-10">
                <p
                  aria-hidden="true"
                  className="font-display text-5xl leading-none text-[var(--color-paper-3)] tabular-nums"
                >
                ...
                <div>
                  ...
                  <h2 className="mt-2 font-display text-3xl leading-tight uppercase sm:text-4xl">
                    {s.title}
                  </h2>
